...
namespace FlightControlWeb.Controllers
{
    ...
    public class FlightPlanController : ControllerBase
    {
        ...
        public async Task<ActionResult> Post(FlightPlanDTO flightPlanDTO)
        {
                ...
                string lastId = !_context.FlightPlans.Include(item => item.Flight).OrderByDescending(item => item.Flight.FlightIdentifier).Any() ? "AAAA-0000" : _context.FlightPlans.Include(item => item.Flight).OrderByDescending(item => item.Flight.FlightIdentifier).First().Flight.FlightIdentifier;
